...
export class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.textRetro = this.add.bitmapText(20, 20, 'knighthawks', 'SCORE\n0', 20).setDepth(3);
        ...
        this.pauseOverlayText = this.add.bitmapText(
        ...
        this.physics.add.overlap(this.spaceship, this.asteroids, (ship, asteroidObj) => {
            ...
            let explosion = this.add.sprite(spaceship.x, spaceship.y, "explosion_sprite");
            ...
            const message = this.add.bitmapText(width / 2, height / 2, 'knighthawks', 'CUIDADIN !!!', 30).setOrigin(0.5).setDepth(10);
            message.setDropShadow(2, 2, 0x000000, 1);
            
            const messageLives = this.add.bitmapText(width / 2, height / 2 + 40, 'knighthawks', 'VIDAS: ' + (3 - this.livesDead), 30).setOrigin(0.5).setDepth(10);
        ...
        this.physics.add.overlap(this.bullets, this.asteroids, (bullet, asteroidObj) => {
            ...
            let explosion = this.add.sprite(bulletImpact.x, bulletImpact.y, "explosion_sprite");
            explosion.play("explosion");
            explosion.setDepth(11);
            ...
            const fadingText = this.add.text(asteroid.x, asteroid.y, '+2', {
                font: '16px Arial',
                color: "#00ff00"
            ...
            if (!asteroid.marker) {
                ...
                const impactText = this.add.text(0, 0, '', {
                    font: "16px Arial",
                    color: "#00ff00",
                    padding: { x: 4, y: 2 }
            ...
            if (asteroid.impactsLeft <= 0) {
                ...
                let explosion = this.add.sprite(asteroid.x, asteroid.y, "explosion_sprite");
                explosion.play("explosion");
                explosion.setDepth(11);
                explosion.setScale(0.8);
    ...
    override update(time: number) {
        ...
        this.asteroids.getChildren().forEach((asteroidObj) => {
            ...
            if (asteroid.marker) {
                ...
                g.strokeLineShape(new Phaser.Geom.Line(x - halfW, y - halfH, x - halfW + size, y - halfH));
                g.strokeLineShape(new Phaser.Geom.Line(x - halfW, y - halfH, x - halfW, y - halfH + size));
                ...
                g.strokeLineShape(new Phaser.Geom.Line(x, y - halfH, x, y - halfH - 20));
                g.strokeLineShape(new Phaser.Geom.Line(x, y - halfH - 20, x + 20, y - halfH - 20));


                // Texto con impactos restantes
                if (asteroid.impactText) {
                    asteroid.impactText.setPosition(x + 80, y - halfH - 20);                   
                    asteroid.impactText.setText("[Impacts:" + (5 - asteroid.impactsLeft) + "/5]");
                }
            }
        });
